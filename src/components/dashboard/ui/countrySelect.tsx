  ...
  <hr/>
      <ul role="list" className="divide-y divide-gray-100">
        {countries.map((countries, index) => (
          <li key={index} className="flex items-center justify-between gap-x-8 py-4">
            <div className="flex min-w-0 gap-x-4">
            <div className="h-10 w-10 flex items-center justify-center rounded-full bg-gray-200">
            <img className="h-5 w-5 border-spacing-8 rounded-full bg-gray-50" src={countries.imageUrl} alt="" />
          </div>
                <p className="text-sm font-bold leading-10 text-gray-900">{countries.name}</p>
                {/* <p className="mt-1 truncate text-xs leading-5 text-gray-500">{product.brand}</p> */}
               
            </div>
            <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
              <p className="text-sm leading-6 font-semibold text-gray-900">{countries.population}</p>
            </div>
          </li> 
        ))}
      </ul>
